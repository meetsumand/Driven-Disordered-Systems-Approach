...
program main
  ...
  double precision::reacount(1:xnm)=0.d0,nmut(1:xnm)=0.d0,ftns(1:xnm)=0.d0,xpts(1:xnm)=0.d0,nmut2(1:xnm)=0.d0
...
do ik=1,xnm
xpts(ik)=1.d0*N*1.d0*((1.d0*ik)/(1.d0*xnm))
enddo
...
!********** Finding minimun of x_i^+ and maximum of x_i^-
   do j=1,N
	if (str(j)==0) then 
...
flpdni=lsi(sslabel(ini),1)
ini=dec(bitflip(bitstr(ini,N),flpdni,N),N)
x2=xplus(sslabel(ini))
!print*,ini,x2,xminus(sslabel(ini))
do ik=1,xnm
flt=0
if ((x2.ge.xpts(ik)).and.(x.le.xpts(ik))) flt=1
if (ini==2**n-1.and.xpts(ik)>x) flt=1
...
ftns(ik)=ftns(ik)+fitness(r0(ini),mic0(ini),xpts(ik))
reacount(ik)=reacount(ik)+1.d0
!print*,ini,ik,rcntr,x,x2,xpts(ik),xminus(sslabel(ini))
!if (reacount(ik)>1.d0*rea) print*,"here",ini,x,x2,ik,xpts(ik),rcntr,reacount(ik)
!print*,"ik x xl xu mut",ik,xpts(ik),x,x2,ini
...
do ik=1,xnm
nmut2(ik)=(nmut2(ik)-nmut(ik)**2)/sqrt(1.d0*rea)
enddo

do ik=1,xnm 
print*,xpts(ik),nmut(ik),nmut2(ik),ftns(ik)/(1.d0*rea),reacount(ik)
