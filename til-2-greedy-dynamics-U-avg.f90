...
program main
  ...
  integer,parameter::N=20,NN=20,kexp=1,ns=200,npop=1000,xnm=1000
  double precision::reacount(1:xnm)=0.d0,nmut(1:xnm)=0.d0,nmut2(1:xnm)=0.d0,ftns(1:xnm)=0.d0,xpts(1:xnm)=0.d0  
  ...
  integer::str(1:N)=0,seed=-9071046,npk=0,nstr(1:N)=0,pkindicator(0:2**n-1)=0,str1(1:N)=0,str2(1:N)=0
  double precision::rx1,mx1,rx2,mx2,xint(1:n*2**(n-1))=0.d0,epsmin=0.d0,xiplusmax=0.d0,xi2,fcurrent=0.d0,fcompare=0.d0
  integer::nbrr=0,ilis=0,start=0,inistep=0,flt=0
  ...
  double precision::snmut(1:xnm)=0.d0,snmut2(1:xnm)=0.d0
...
do ik=1,xnm
xpts(ik)=(exp(1.d0))**(2.d0*N*ik/(1.d0*xnm)-6.d0)
...
! print*,"x0plus",x0plus
   ...
   enddo
if (i==0) x0minus=0.d0
if (i==2**N-1) x0plus=(x0minus)*10.d0
...
do ik=1,xnm
flt=0
if ((x2.ge.xpts(ik)).and.(x.le.xpts(ik))) flt=1
if (ini==2**n-1.and.(xpts(ik)>x)) flt=1
if (flt==1) then 
nmut(ik)=nmut(ik)+1.d0*strsize(bitstr(ini,N),N)
nmut2(ik)=nmut2(ik)+1.d0*((strsize(bitstr(ini,N),N))**2)

ftns(ik)=ftns(ik)+fitness(r0(ini),mic0(ini),xpts(ik))
reacount(ik)=reacount(ik)+1.d0
msig(ik)=msig(ik)+mic0(ini)
msig2(ik)=msig2(ik)+1.d0*mic0(ini)**2
rsig(ik)=rsig(ik)+r0(ini)
...
sx=x/mic0(ini)
sx2=x2/mic0(ini)
if ((sx2.ge.sxpts(ik)).and.(sx.le.sxpts(ik))) flt=1
if (ini==2**n-1.and.sxpts(ik)>sx) flt=1
if (flt==1) then 
snmut(ik)=snmut(ik)+strsize(bitstr(ini,N),N)
snmut2(ik)=snmut2(ik)+1.d0*((strsize(bitstr(ini,N),N))**2)
sreacount(ik)=sreacount(ik)+1.d0
!print*,"ik x xl xu mut",ik,xpts(ik),x,x2,ini
endif
...
do while(ini.ne.2**n-1)!************************* start of dynamics

flg=0
!dx=0.000005d0*(1.d0+xplus(sslabel(ini)))


x=xplus(sslabel(ini))!+dx
...
inistep=ini
!print*,"ini",ini,fitness(r0(ini),mic0(ini),x)
   do while (flg==0)
  !start=start+1 
  ini=fnbr
!print*,"ini",ini
...
if (fnbr==ini2) then 
print*,"attention",ini2,epsmin,rcntr
!print*,"xplus",xplus
exit
endif
...
if (ini==2**n-1.and.sxpts(ik)>sx) flt=1
if (flt==1) then 
snmut(ik)=snmut(ik)+1.d0*strsize(bitstr(ini,N),N)
snmut2(ik)=snmut2(ik)+1.d0*((strsize(bitstr(ini,N),N))**2)
sreacount(ik)=sreacount(ik)+1.d0
!print*,"ik x xl xu mut",ik,xpts(ik),x,x2,ini
endif
...
nmut(ik)=nmut(ik)/reacount(ik)
nmut2(ik)=nmut2(ik)/reacount(ik)
nmut2(ik)=sqrt(nmut2(ik)-nmut(ik)**2)/(sqrt(reacount(ik)))

snmut(ik)=snmut(ik)/sreacount(ik)
snmut2(ik)=snmut2(ik)/sreacount(ik)
snmut2(ik)=sqrt(snmut2(ik)-snmut(ik)**2)/(sqrt(sreacount(ik)))


msig(ik)=msig(ik)/reacount(ik)
msig2(ik)=msig2(ik)/reacount(ik)
msig2(ik)=sqrt(msig2(ik)-msig(ik)**2)/(sqrt(reacount(ik)))

rsig(ik)=rsig(ik)/reacount(ik)

ftns(ik)=ftns(ik)/reacount(ik)
enddo


do ik=1,xnm 
print*,xpts(ik),nmut(ik),nmut2(ik),msig(ik),msig2(ik),rsig(ik),sxpts(ik),snmut(ik),snmut2(ik),ftns(ik)
enddo
